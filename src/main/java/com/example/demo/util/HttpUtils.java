...
import com.example.demo.exception.BasicWebBasicCodeEnum;
...
public class HttpUtils {
     ...
     * 预编译正则语句加快匹配速度
     */
    private static Pattern patternForGetParam = Pattern.compile("(req|param)=\\{(.+:.+)+}");

    private static Pattern patternForPostParam = Pattern.compile("(([a-zA-Z0-9_]+)=([^&]+))&?+");
    ...
    public static String getRequestBody(String requestBody) {
        ...
        Matcher matcher = patternForGetParam.matcher(requestBody);
        if (matcher.find()) {
            return requestBody.replaceFirst("(req|param)=","");
        }

        JSONObject requestBodyJSON = null;
        matcher = patternForPostParam.matcher(requestBody);
        while (matcher.find()) {
    ...
    public static void main(String[] args) {
        ...
        JSONObject requestBodyJSON = null;
        Matcher matcher = patternForPostParam.matcher(requestBody);
        while (matcher.find()) {
            if (requestBodyJSON == null) {
                requestBodyJSON = new JSONObject();
            }

...
@Slf4j
public class RedisConcurrentDelayQueue implements DelayQueue {
    ...
    public RedisConcurrentDelayQueue(String queueName, long delay, boolean autoAck) {
        this.queueName = QUEUE_PREFIX + queueName;
        this.exeSetKey = EXECUTION_KEYSET_PREFIX + queueName;
        this.autoAck = autoAck;
        this.delay = delay * 1000 *1000;
        jedisPool = new RedisClient().getJedisPool();

        RedisDistributedLock lock = new RedisDistributedLock( 60000);
                ...
                if (result != null) {
                    ...
                    if (delayMessage != null) {
                        if (delayMessage.getExpire() > System.nanoTime()) {
                            ...
                            log.info("等待时间: {} 秒",waitTime / 1000000000L);
                        }else{
